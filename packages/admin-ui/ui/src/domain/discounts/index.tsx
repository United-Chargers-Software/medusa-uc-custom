...
const DiscountIndex = () => {
  ...
  useEffect(() => {
    const updateExpiredDiscounts = async () => {
      ...
      for (const d of expiredDiscounts) {
        try {
          console.log('Updating discount:', d.id);

          await client.admin.discounts.update(d.id, {
            is_disabled: true,
          });
        } catch (error) {
          console.error(`Error updating discount ${d.id}:`, error);
        }
      }
    };
    ...
    <div className="flex h-full flex-col">
      <div className="gap-y-xsmall flex w-full grow flex-col">
        {getWidgets('discount.list.before').map((w, index) => {
          return <WidgetContainer key={index} widget={w} injectionZone="discount.list.before" entity={null} />;
        })}
        ...
        {getWidgets('discount.list.after').map((w, index) => {
          return <WidgetContainer key={index} widget={w} injectionZone="discount.list.after" entity={null} />;
        })}
        <Spacer />
      </div>
  ...
  return (
    <Routes>
      ...
      {nestedRoutes.map((r, i) => {
        return <Route path={r.path} key={i} element={<RouteContainer route={r} previousPath={'/discounts'} />} />;
      })}
    </Routes>
  );
};
