...
const SummaryCard: React.FC<SummaryCardProps> = ({ order, reservations }) => {
  ...
  const { state: reservationModalIsOpen, open: showReservationModal, close: closeReservationModal } = useToggleState();
        ...
        new Map(
          inventory
            .filter(
              ...
              ): inventoryItem is Response<AdminGetVariantsVariantInventoryRes> => !!inventoryItem,
            )
            .map(i => {
  ...
  const reservationItemsMap = useMemo(() => {
    ...
    return reservations.reduce((acc: Record<string, ReservationItemDTO[]>, item: ReservationItemDTO) => {
      if (!item.line_item_id) {
        return acc;
      }
      acc[item.line_item_id] = acc[item.line_item_id] ? [...acc[item.line_item_id], item] : [item];
      return acc;
    }, {});
  ...
  const allItemsReserved = useMemo(() => {
    return order.items.every(item => {
      if (!item.variant_id || !variantInventoryMap.get(item.variant_id)?.inventory.length) {
        return true;
      }
      ...
      return (
        ...
        (reservations && sum(reservations.map(r => r.quantity)) === item.quantity - (item.fulfilled_quantity || 0))
      );
    });
    ...
    >
      <div className="mt-6">
          ...
          <DisplayTotal
            ...
            totalTitle={
              <div className="inter-small-regular text-grey-90 flex items-center">
                ...
                <div className="ml-2">
                  <CopyToClipboard value={gcTransaction.gift_card.code} showValue={false} iconSize={16} />
                </div>
              </div>
            }
          />
        ...
        <DisplayTotal
          ...
          totalTitle={
            hasMovements ? t('detail-cards-original-total', 'Original Total') : t('detail-cards-total', 'Total')
          }
        />
        <PaymentDetails
