...
const sortByOptions: SortOption[] = [{ value: 'created_at', label: 'Date added' }];
...
const OrderSortDropdown = ({ sortBy, sortDirection, onSortChange }: OrderSortProps) => {
  ...
  const getCurrentSortLabel = () => {
    if (!sortBy || !sortDirection) {
      return t('order-sort-dropdown-sort-by', 'Sort by');
    }
    ...
    >
      <div className="w-full">
        <div className="border-grey-20 border-b px-3 py-2 text-xs font-semibold">
          {t('order-sort-dropdown-sort-by', 'Sort by')}
        </div>
        <div className="py-2">
        ...
        <div className="border-grey-20 border-t border-b px-3 py-2 text-xs font-semibold">
          {t('order-sort-dropdown-direction', 'Direction')}
        </div>
        <div className="py-2">
          <RadioGroup.Root
