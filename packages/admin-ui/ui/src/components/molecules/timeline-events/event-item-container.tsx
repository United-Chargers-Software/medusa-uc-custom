...
const EventItemContainer: React.FC<EventItemContainerProps> = ({ item, detail }) => {
  if (!item) {
    return null;
  }
      ...
      <div className="gap-x-small flex items-center">
        {item.thumbnail && (
          <div className="rounded-base h-10 w-[30px] overflow-hidden">
            <img src={item.thumbnail} alt={`Thumbnail for ${item.title}`} className="h-full w-full object-cover" />
          </div>
        )}
